...
def train(config):
    ...
    logging.info(f'Using gradient_accumulation_steps={gradient_accumulation_steps}')
    ...
    assert os.path.exists(dataset.fid_stat), '{}'.format(dataset.fid_stat)
    ...
    autoencoder = get_model(config.autoencoder.pretrained_path)
    ...
    def train_step(_batch):
        ...
        _z = _z.to(dtype)
        loss = sde.LSimple(score_model, _z, pred=config.pred, y=y)
