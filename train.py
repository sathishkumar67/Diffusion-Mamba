...
from mamba_attn_diff.utils.backup_code import backup_code
...
def train(config):
    ...
    if accelerator.is_main_process:
        ...
        backup_code(os.path.abspath(workdir))

        wandb.init(dir=os.path.abspath(workdir), 
                   project=f'uvit_{config.dataset.name}', config=config.to_dict(),
    ...
    scheduler_config = dict(name='vpsde') if not config.get('scheduler', False) else config.scheduler
    scheduler = get_sde(device=device, **scheduler_config) # sde.VPSDE()
    scheduler_ema = get_sde(device=device, **scheduler_config)
    score_model = sde.ScoreModel(nnet, pred=config.pred, sde=scheduler)
    score_model_ema = sde.ScoreModel(nnet_ema, pred=config.pred, sde=scheduler_ema)


    def train_step(_batch, iter_rate):
        _metrics = dict()
        optimizer.zero_grad()
        if config.train.mode == 'uncond':
            loss = sde.LSimple(score_model, _batch, pred=config.pred, iter_rate=iter_rate)
        elif config.train.mode == 'cond':
            loss = sde.LSimple(score_model, _batch[0], pred=config.pred, y=_batch[1], iter_rate=iter_rate)
        else:
    ...
    def eval_step(n_samples, sample_steps, algorithm):
        ...
        def sample_fn(_n_samples):
            ...
            elif algorithm in ['edm', 'ddim', 'ddpm']:
                return sde.diffusers_denoising(
                    score_model_ema, scheduler.noise_scheduler, _x_init, 
    ...
    while train_state.step < config.train.n_steps:
        ...
        metrics = train_step(batch, train_state.step*1. / config.train.n_steps)
        ...
        if train_state.step % config.train.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                if config.sample.algorithm in ['edm', 'ddim', 'ddpm']:
                    if config.train.mode == 'uncond':
                        samples = sde.diffusers_denoising(
                            score_model_ema, scheduler.noise_scheduler, x_init=x_init, 
                            sample_steps=config.sample.sample_steps, device=device,)
                    elif config.train.mode == 'cond':
                        samples = sde.diffusers_denoising(
                            score_model_ema, scheduler.noise_scheduler, x_init=x_init, 
                            sample_steps=config.sample.sample_steps, device=device, y=y, 
                            do_classifier_free_guidance=True, cfg_weight=1.5)
                else:
                    if config.train.mode == 'uncond':
        ...
        if train_state.step % config.train.save_interval == 0 or train_state.step == config.train.n_steps:
            ...
            fid = eval_step(n_samples=10000, sample_steps=50, 
                algorithm=(config.sample.algorithm if config.get('scheduler', False) else 'dpm_solver') )  # calculate fid of the saved checkpoint
